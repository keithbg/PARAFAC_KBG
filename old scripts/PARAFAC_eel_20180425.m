...
max_components = 8; %This is the maximum number of components there might possibly be in the model (does NOT mean the model would ultimately be run with this number of components). I recommend using 10 here, but if the best model turns out to be the 10-component model, rerun the analysis with a larger max_components.
...
OriginalDataNorm = normeem(OriginalData);
...
possible_outliers = [88 115 65 91]; %Enter a vector of the sample numbers that are possibly outliers, separated by spaces. If there are none, set possible_outliers equal to [].
...
%% ENTER ADDITIONAL INPUT DATA
final_outliers.samples = [167 224]; %Enter a vector of the sample numbers that are outliers that should be removed. If there are none, set final_outliers.samples to [].
...
if isempty(possible_outliers)+isempty(ind_em)+isempty(ind_ex)<3 %Do this if there are outliers to remove
    [CutData2] = RemoveOutliers(input_cut_data, possible_outliers, ind_em, ind_ex); %Remove the outliers from the sample matrix
    if ~isempty(desig) && ~isempty(possible_outliers) %Do the next line if desig is not an empty array
        desig2 = input_desig(setdiff(1:length(input_desig), possible_outliers)); %Remove the outliers from the designator matrix.
    %end
    %PERFORM AN OUTLIER ANALYSIS
...
save('CutData3File.mat', 'CutData2')
save('TestCut3File.mat', 'TestCut2')
...
for ii = 2:max_components
    ...
    %PlotLL(TestCut2, ii); %Plot leverage and loadings plots
    %ComponentEEM(TestCut, ii); %Plot EEMs of components
    ComponentEEM_save_KBG(TestCut, ii, '/Users/KeithBG/Documents/UC Berkeley/2016 Spring Classes/EEA_DOC/Aqualog_EEMs/Figures/Components/TestCut')
    %pause
end
